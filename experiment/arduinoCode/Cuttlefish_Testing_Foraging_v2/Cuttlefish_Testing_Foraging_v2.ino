...
int bonsaiGoPin = 3;
int bonsaiStopPin = 4;
boolean button0Val, button0Prev;
...
void setup() {
  ...
  */
  ...
  Serial.println("Press button to start Foraging test.  Hold button down for 2s to stop test.");   
...
void ccw360() {
  ...
  delay(1000);         // Assumuming rotates at 1Hz
  moveServo(0, 511);  // Move the servo on channel 0 to an angle of 0 degrees
}
...
void cw180() {
  ...
  moveServo(0, pulseLow);   // Move the servo on channel 0 to an angle of -45 degrees
  delay(530);            // Assumuming rotates at 1Hz
  moveServo(0, 511);    // Move the servo on channel 0 to an angle of 0 degrees
}
...
/*  bonsaiGoState=digitalRead(bonsaiGoPin);
  ...
  if(currentTime > tButtonPoll+10) {
    ...
    if(button0Val==LOW && button0Prev==HIGH) {  //Determine if button just pressed.
      testStarted=1;  // Notes that the test has been manually started.
      foraging();
    }
...
void foraging() {
  ...
  delay(2000);
  cw180();
  numWiggles = 1800;        // "numWiggles" = how long in [Seconds] the food wiggles 
  /*
  numWiggles = 10;
  numWiggles = 6;
  numWiggles = 2;
  */
  for(j=0; j<numWiggles; j++) {
    ...
    //if button held down, break out of loop
    if(button0Val==LOW && button0Prev==HIGH) {
      testStarted=0;
      digitalWrite(cueLED, LOW);  
      cw180();
      stopServoPulses();
      Serial.println("Successful Catch!  Press button to start next foraging trial.");    
      break;  
    }
    /*
    bonsaiStopState=digitalRead(bonsaiStopPin);
